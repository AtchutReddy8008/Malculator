...
class Engine:
    ...
    def find_hedge_strike(self, short_strike: int, cp: str, common_target_prem: float, simulate: bool = False) -> int:
        ...
        # Smarter fallback — find nearest OTM strike that EXISTS in weekly_df
    ...
    def enter(self) -> bool:
        ...
        # ══════════════════════════════════════════════════════════════════════
        # STEP 1 — ACQUIRE DB ENTRY LOCK (MUST BE FIRST, BEFORE ANY API CALL)
        #
        # This is the ONLY place entry_attempted_date is set.
        # Using select_for_update() inside a short atomic block ensures that
        # even if the main loop calls enter() twice concurrently (e.g. because
        # _entry_guard_seconds elapsed during slow API calls), only one will
        # ever pass this gate per day. The second one sees the date already
        # set and returns False immediately — before any API call or order.
        #
        # Why not put this inside a broader atomic block around the whole
        # function? Because the order placement calls (kite.place_order) are
        # external HTTP calls that MUST NOT run inside a DB transaction. A long
        # transaction holding a row lock while waiting for Zerodha's API would
        # block all other DB access to that row and risk deadlocks.
        # ══════════════════════════════════════════════════════════════════════
        try:
            with transaction.atomic():
                ...
                if bot_status_lock.entry_attempted_date == today:
                    ...
                    return False
                # Lock acquired — write immediately so any parallel call sees it
            ...
            return False
        # ══════════════════════════════════════════════════════════════════════
        # From here on: lock is held in DB. Any failure path should NOT release
        # it — we want the day to remain locked even on partial failures to
        # avoid a retry that could double-enter on a day where orders may have
        # partially filled.
        # ══════════════════════════════════════════════════════════════════════
        ...
        # Update last_successful_entry on BotStatus (entry_attempted_date already set at top)
        try:
...
class TradingApplication:
    def __init__(self, user, broker):
        ...
        # Entry guard must exceed the entry window duration (90s) to prevent
        # the loop from calling enter() a second time during slow API calls.
        # Set to 120s (entry window is 9:21:00–9:22:30 = 90s).
            ...
            while self.running:
                ...
                if dtime(9, 19) <= current_time < dtime(9, 20) and not self._early_0919_logged:
                    ...
                    try:
                        ...
                        self._early_0919_logged = True  # mark done even on failure to avoid spam
                ...
                if dtime(9, 0) <= current_time < dtime(9, 30) and not self._snapshot_logged:
                    try:
                        ...
                        self._snapshot_logged = True  # mark done even on failure
