...
class Config:
    ...
    UNDERLYING = "NIFTY"
    LOT_SIZE = 65
    ENTRY_START = dtime(11, 39, 0)
    ENTRY_END = dtime(11, 40, 30)
...
class DBLogger:
    ...
    def _write(self, level: str, msg: str, details: dict = None):
        ...
        # FIX: Always flush and always print details so they appear in Celery logs
...
class Engine:
    ...
    def load_weekly_df(self):
        """
        NIFTY weekly expiry is THURSDAY. Find the nearest upcoming Thursday expiry.
        Falls back to the nearest available expiry if Thursday is a holiday.
        """
        ...
        try:
            ...
            # Find the nearest available expiry >= today
            target_expiry = None
            ...
            self.logger.info("Weekly cache updated", {
                ...
                "strikes_available": sorted(self.weekly_df["strike"].unique().tolist())[:10]  # FIX: log sample strikes
            })
        except Exception as e:
    ...
    def find_short_strike(self, atm_strike: int, cp: str) -> int:
        """
        FIX: Added detailed logging of available strikes and LTP values so we can
        diagnose why strikes are being skipped. Also reduced min_premium guard
        from 2.0 to 0.5 for the initial target search to avoid missing valid strikes.
        """
        ...
        # FIX: Log available strikes near target so we can see what's in weekly_df
        ...
        if sym:
            ...
            if prem > 0.5:  # FIX: lowered from 2.0 — any tradeable premium is fine
        ...
        # FIX: Wider offset search and lower premium floor
        for offset in [50, -50, 100, -100, 150, -150, 200, -200]:
            test_strike = target + offset
            # Skip strikes on wrong side of ATM
            ...
            if sym:
                ...
                if prem > 0:  # FIX: accept any non-zero premium in fallback
    ...
    def find_hedge_strike(self, short_strike: int, cp: str, common_target_prem: float, simulate: bool = False) -> int:
        """
        FIX: Relaxed premium guard from 3.0 to 0.5. Also widen the overpay
        multiplier search so we don't fail to find any hedge at all.
        """
        direction = 1 if cp == "CE" else -1
        ...
        for _, row in df.iterrows():
            ...
            if prem <= 0.5:  # FIX: was 3.0 — too restrictive, OTM hedges can be cheap
                continue
            if prem > common_target_prem * Config.MAX_OVERPAY_MULT * 2:  # FIX: 2x buffer for low target premiums
                continue
        ...
        # FIX: Smarter fallback — find nearest OTM strike that EXISTS in weekly_df
        fallback_strike = None
        if cp == "CE":
            candidates = sorted(df["strike"].unique().tolist())
        else:
            candidates = sorted(df["strike"].unique().tolist(), reverse=True)
    ...
    # 1️⃣ Get frontend-selected lot size
        try:
        ...
        # 2️⃣ Check minimum capital requirement
        ...
        # 3️⃣ Check required margin for requested lots
        try:
    ...
    def preview_profit_calculation(self):
        try:
            ...
            # FIX: preview crash must never kill the bot
    ...
    def startup_banner(self):
        try:
            ...
            # FIX: banner crash must never kill the bot
    ...
    def enter(self) -> bool:
        ...
        # FIX: Do NOT use @transaction.atomic() on the whole enter() function.
        # The Celery task's DB connection holds locks (from heartbeat saves) and
        # select_for_update() inside an outer atomic block causes a deadlock.
        # Instead, do a quick non-locking pre-check here, then lock only at commit time.
        bot_status = BotStatus.objects.get(user=self.user)
        if bot_status.entry_attempted_date == today:
            self.logger.critical("ENTRY ALREADY ATTEMPTED TODAY - locked by DB (pre-check)")
            return False
        ...
        # FIX: Log exactly what we got so we can diagnose any None symbols
        ...
        # FIX: Do NOT block entry if hedges are missing — hedges are protection, not mandatory.
        # Warn heavily but continue without hedges if symbols are not found.
        if not ce_hedge_sym:
        ...
        # FIX: Lock the day BEFORE placing any orders.
        # Previously this was done AFTER orders filled, meaning the loop could
        # re-enter during the ~20-30s order placement window and fire duplicate legs.
        with transaction.atomic():
            bot_status_lock = BotStatus.objects.select_for_update().get(user=self.user)
            if bot_status_lock.entry_attempted_date == today:
                self.logger.critical("ENTRY LOCK ALREADY SET - aborting to prevent duplicate trade")
                return False
            bot_status_lock.entry_attempted_date = today
            bot_status_lock.save(update_fields=['entry_attempted_date'])
        self.logger.critical("ENTRY LOCK SET IN DB - proceeding to place orders")
        ...
        with ThreadPoolExecutor(max_workers=len(orders)) as executor:
            ...
            for future in as_completed(futures_map):
                ...
                try:
                    ...
                    self.logger.error(f"Order thread raised exception for {sym}", {"error": str(e), "trace": traceback.format_exc()})
        ...
        # entry_attempted_date already set before orders were placed — no action needed here.
    ...
    def _emergency_square_off(self, reason="Emergency"):
        """Only place ONE order per position, not two."""
    ...
    def save_periodic_pnl_snapshot(self):
        # Only update BotStatus heartbeat fields; do not write unrealized to DailyPnL
        pass
...
class TradingApplication:
    def __init__(self, user, broker):
        ...
        # FIX: reduced from 60s to 20s — entry window is only 90s, 60s guard was too aggressive
    ...
    def _check_is_running(self) -> bool:
        """
        Robust is_running check with retry on DB failure.
        Returns True if bot should keep running, False if it should stop.
        """
        try:
            bot_status = BotStatus.objects.get(user=self.user)
            ...
            # Return True on DB failure so a transient DB blip doesn't kill the bot
            return True
    ...
    def run(self):
        ...
        # FIX: Give the DB time to commit is_running=True before the first read.
        time.sleep(3)
        ...
        try:
            while self.running:
                ...
                if dtime(9, 19) <= current_time < dtime(9, 20) and not self._early_0919_logged:
                    ...
                    try:
                        ...
                        self._early_0919_logged = True  # FIX: mark done even on failure to avoid spam
                ...
                if dtime(9, 0) <= current_time < dtime(9, 30) and not self._snapshot_logged:
                    try:
                        ...
                        self._snapshot_logged = True  # FIX: mark done even on failure
                    ...
                    while attempts < Config.MAX_TOKEN_ATTEMPTS:
                        ...
                        try:
                            ...
                            if access_token:
                                ...
                                # FIX: Wrap startup_banner in try-except so a banner
                                # crash cannot kill the token refresh success path
                                try:
                                    self.engine.startup_banner()
                        ...
                        self.logger.critical("Token refresh failed after all attempts - stopping bot")
                        self.running = False
                        break
                ...
                if self.engine.instruments is not None and self.engine.weekly_df is not None:
                    if self.engine.state.data["trade_active"]:
                        ...
                        if Config.ENTRY_START <= current_time <= Config.ENTRY_END:
                            ...
                            now_ts = time.time()
                            # FIX: guard reduced from 60s to 20s for 90s entry window
                            if now_ts - self._last_entry_check_time < self._entry_guard_seconds:
                                self.logger.info(f"Entry guard active — skipping this cycle ({self._entry_guard_seconds:.0f}s cooldown)")
                                time.sleep(5)
                                continue
                            ...
                            try:
                                ...
                                if success:
                                    ...
                                    self.logger.warning("Entry attempt failed — will retry if still in window")
                                    time.sleep(10)
                            except Exception as e:
                ...
                if current_time >= Config.MARKET_CLOSE and not self._daily_summary_saved:
                    try:
                        ...
                        self.logger.big_banner(f"MARKET CLOSE SUMMARY | Unrealized was: ₹{unrealized:,.2f} (not saved to DailyPnL)")
                        self._daily_summary_saved = True
                    except Exception as e:
                        self.logger.error("Market close summary failed", {"error": str(e)})
                        self._daily_summary_saved = True  # FIX: prevent infinite retry
            ...
            self.logger.critical("Fatal error in main loop", {
                "error": str(e),
                "error_type": type(e).__name__,
                "trace": traceback.format_exc()  # FIX: always include full traceback in details
            })
        finally:
            self.running = False
