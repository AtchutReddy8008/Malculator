...
class BrokerAdmin(admin.ModelAdmin):
    list_display = ('user', 'broker_name', 'is_active', 'created_at')
    list_filter = ('broker_name', 'is_active')
    search_fields = ('user__username', 'zerodha_user_id')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        ('User Info', {'fields': ('user', 'broker_name')}),
        ('Zerodha Credentials', {'fields': ('api_key', 'secret_key', 'zerodha_user_id', 'password', 'totp')}),
...
class BotStatusAdmin(admin.ModelAdmin):
    list_display = ('user', 'is_running', 'last_started', 'last_stopped', 'current_unrealized_pnl')
